...
**Base URL:** `http://localhost:12400` (or your production server URL)
...
**Example (curl):**
```bash
curl -X POST http://localhost:12400/api/register \
...
```bash
TOKEN="eyJhbGciOiJIUzI1NiIs..."
curl http://localhost:12400/api/characters \
...
The server allows requests from `CLIENT_ORIGIN` (defaults to `http://localhost:12000` for dev).
